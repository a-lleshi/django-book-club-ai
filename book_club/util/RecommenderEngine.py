""" Recommender Engine using User-based Collaborative Filtering"""
...
def getMatch(rating):
    """ Returns a match value for a given rating """
    if int(rating) == 10:
        return 100
    x = 80
...
def getTopNRecs(club_id, k=10):
    ...
    for item_id, rating_sum in sorted(candidates.items(), key=itemgetter(1), reverse=True):
        if not item_id in books_read:
            ...
            try:
                books = models.Book.objects.filter(ISBN=book_id)
                for b in books:
                    book = b
                    break
                match = getMatch(rating_sum*10)
                recommended_books.append((book, match))
