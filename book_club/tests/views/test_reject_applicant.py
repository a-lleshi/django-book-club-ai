...
from book_club.views.club_application_views import reject_applicant

class RejecttApplicantsTestCase(TestCase):
    """Unit tests for the accepting applicants."""
    ...
    def setUp(self):
        ...
        self.url = reverse(reject_applicant, kwargs={'club_id': self.club.id, 'user_id': self.applicant.id})

    def test_accept_applicants_url(self):
        correct_url = f'/reject_applicant/1&1'
        self.assertEqual(self.url, correct_url)

    def test_applicant_is_rejected_by_owner(self):
        self.client.login(email=self.owner.email, password='Password123')
        applicantBefore = Membership.objects.get(user=1, club=1)
        response = self.client.post(self.url, follow = True)
        applicantAfter = Membership.objects.filter(user=1, club=1).exists()
        self.assertEqual(applicantBefore.level,'APP')
        self.assertEqual(False, applicantAfter)

    def test_applicant_is_rejected_by_officer(self):
